<template>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
        <div>
            <h3 class="text-lg"><b>Edit Committee Position</b></h3>
        </div>
    ...
    <div class="py-4 px-4 mb-5 rounded-md shadow-md">
        <form class="space-y-6" @submit.prevent="editCommitteePosition">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4">
                <div class="pb-1 col-span-2">
                    <label for="position_title " class="block text-md font-medium text-gray-700">Position Title  <span class="text-red-500">*</span></label>
                    <div class="mt-1">
                        <input type="text" name="position_title " id="position_title "
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                v-model="committeeposition.position_title ">
                        <span class="text-sm text-red-600" v-if="errors?.position_title ">{{ errors.position_title[0] }}</span>
                    </div>
                </div>
                <div class="pb-1">
                    <label for="is_active_position" class="block text-md font-medium text-gray-700">Status</label>
                    <div class="mt-1">
                        <select name="is_active_position" id="is_active_position" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        v-model="committeeposition.is_active_position">
                            <option value="">-Select Status-</option>
                            <option value="1">Active</option>
                            <option value="0">Inactive</option>
                        </select>
                        <span class="text-sm text-red-600" v-if="errors?.is_active_position">{{ errors.is_active_position[0] }}</span>
                    </div>
                </div>
                <div class="pb-1 pt-8">
...
const { errors, committeeposition, getCommitteePosition, updateCommitteePosition } = useCommitteePositions()
...
onMounted(() => getCommitteePosition(props.id))

const editCommitteePosition = async () => {
    await updateCommitteePosition(props.id)
}
</script>