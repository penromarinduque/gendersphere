    ...
    <div class="py-4 px-4 mb-5 rounded-md shadow-md">
        <form class="space-y-6" @submit.prevent="editGenderIssue">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-5 gap-4">
                ...
                <div class="pb-1">
                    <label for="is_active_gender_issue" class="block text-md font-medium text-gray-700">Status</label>
                    <div class="mt-1">
                        <select name="is_active_gender_issue" id="is_active_gender_issue" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        v-model="genderissue.is_active_gender_issue">
                            <option value="">-Select Status-</option>
                            <option value="1">Active</option>
                            <option value="0">Inactive</option>
                        </select>
                        <span class="text-sm text-red-600" v-if="errors?.is_active_gender_issue">{{ errors.is_active_gender_issue[0] }}</span>
                    </div>
                </div>
            </div>
            <div class="py-4">
...
const { errors, genderissue, yearlist, getGenderIssue, updateGenderIssue, getYearlist } = useGenderIssues()
...
const editGenderIssue = async () => {
    await updateGenderIssue(props.id)
}
</script>