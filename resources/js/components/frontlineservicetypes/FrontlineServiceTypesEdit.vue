<template>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
        <div>
            <h3 class="text-lg"><b>Edit Frontline Service Type</b></h3>
        </div>
        <div class="flex mb-4 place-content-end">
            <button class="inline-flex items-center px-4 py-1 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-red-800 border border-transparent rounded-md hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-gray-300 disabled:opacity-25">
                <router-link :to="{ name: 'frontlineservicetypes.index' }" class="text-sm font-medium">Cancel</router-link>
            </button>
        </div>
    </div> 
    <hr>
                ...
                <div class="pb-1 col-span-2">
                    ...
                    <div class="mt-1">
                        <input type="text" name="service" id="service"
                                ...
                                v-model="frontlineservicetype.service ">
                        <span class="text-sm text-red-600" v-if="errors?.service">{{ errors.service[0] }}</span>
                    </div>
                </div>
                <div class="pb-1">
                    <label for="fs_status" class="block text-md font-medium text-gray-700">Status</label>
                    <div class="mt-1">
                        <select name="fs_status" id="fs_status" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        v-model="frontlineservicetype.fs_status">
                            <option value="">-Select Status-</option>
                            <option value="1">Active</option>
                            <option value="0">Inactive</option>
                        </select>
                        <span class="text-sm text-red-600" v-if="errors?.fs_status">{{ errors.fs_status[0] }}</span>
                    </div>
                </div>
                <div class="pb-1 pt-8">
...
const { errors, frontlineservicetype, getFrontlineServiceType, updateFrontlineServiceType } = useFrontlineServiceTypes()
...
onMounted(() => getFrontlineServiceType(props.id))

const editFrontlineServiceType = async () => {
    await updateFrontlineServiceType(props.id)
}
</script>